...
<h3><span id="stopwatch" class="pulse">⏱️</span> Labs Completed: <span id="labCounter">0</span></h3>

<script>
fetch('/_data/labs.json')
  .then(response => response.json())
  .then(data => {
    const counter = document.getElementById('labCounter');
    const stopwatch = document.getElementById('stopwatch');
    const target = data.completed;
    let count = 0;
  ...
  .catch(err => {
    document.getElementById('labCounter').textContent = '—';
